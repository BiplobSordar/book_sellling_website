...
import cloudinary from './src/config/Cloudnary.config.js';
import { Book } from './src/models/book.model.js'


const imageFolderPath = './books'; // your local folder path
...
export const uploadImageAndSaveBooks = async () => {
  for (const book of books) {
    try {
      const fileNumber = book._id; // ✅ use _id directly
      const localImagePath = path.join(imageFolderPath, `book-${fileNumber}.png`);

      // Check if file exists before uploading
      if (!fs.existsSync(localImagePath)) {
        console.warn(`⚠️ Image not found: book-${fileNumber}.png`);
        continue;
      }

      const result = await cloudinary.uploader.upload(localImagePath, {
        folder: 'books',
        public_id: `book-${fileNumber}`,
      });
      ...
      console.log(`✅ Saved book "${book.title}" with image: ${result.secure_url}`);
    } catch (error) {
      console.error(`❌ Failed for book ID ${book._id}:`, error.message);
    }
  }


  console.log('finally all image uploade and books are saved in the databnase')
};